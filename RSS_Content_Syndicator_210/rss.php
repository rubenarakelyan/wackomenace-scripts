...
*				   rss.php
*			     -------------------
*   begin		 : Monday, September 4, 2006
*   notes		 : This code is based on the work of the original
*			   developers below. Portions of this code have
*			   been 'borrowed' from phpbb_fetch_posts, an
*			   untitled RDF content syndicator posted at
*			   phpbb.com, and phpBB itself.
*   email		 : ruben@wackomenace.co.uk
*			   spaced_monkey@hotmail.com
*
*   $Id: rss.php,v 2.1.0 2006/09/04 19:25:00 ruben Exp $
*
*
***************************************************************************/
...
*   notes		 : This code is based on the work of the original
*			   developer below. Portions of this code have
*			   been 'borrowed' from phpbb_fetch_posts, an
*			   untitled RDF content syndicator posted at
*			   phpbb.com, and phpBB itself.
...
$template->assign_vars(array(
	...
	'BOARD_WEBMASTER' => $board_config['board_email'],
        'BOARD_LANGUAGE' => $feed_language,
        'BOARD_COPYRIGHT' => $feed_copyright,
        'BOARD_IMAGE' => $feed_image,
...
// SQL statement to fetch active posts of public forums
$sql = 'SELECT f.forum_name, t.topic_title, u.user_id, u.username, p.post_id, p.topic_id, pt.post_text, pt.post_subject, pt.bbcode_uid, p.post_time, t.topic_replies, t.topic_first_post_id FROM '.FORUMS_TABLE.' AS f, '.TOPICS_TABLE.' AS t, '.USERS_TABLE.' AS u, '.POSTS_TABLE.' AS p, '.POSTS_TEXT_TABLE.' as pt WHERE
    t.forum_id = f.forum_id
    AND f.auth_view = '.AUTH_ALL.'
    AND p.poster_id = u.user_id
    AND pt.post_id = p.post_id
    AND p.topic_id = t.topic_id
    '.$sql_topics_only_where.$sql_forum_where.'
...
if (!$posts_query) {
	...
	if ($post_subject != '') {
        ...
        // Variable reassignment for author
        ...
        $template->assign_block_vars('post_item', array(
          ...
          'POST_TIME' => gmdate('D, d M Y H:i:s', $post['post_time']).' GMT',
          ...
          'TOPIC_ID' => 'posting.php?mode=reply&amp;t='.$post['topic_id']
          )
        );
    }
}

// Add XML and no-caching headers (copied from page_header.php)
